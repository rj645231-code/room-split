...
exports.updateProfile = async (req, res) => {
  const { name, username, color, avatar, upiId, dietary, dislikes, allergies, budget_start_day } = req.body;
  try {
    const userRow = await db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.id);
    ...
    await db.prepare(`
      UPDATE users
      SET name = ?, username = ?, color = ?, avatar = ?, upi_id = ?, dietary = ?, dislikes = ?, allergies = ?, budget_start_day = ?
      WHERE id = ?
    `).run(
      name || userRow.name,
      ...
      upiId !== undefined ? upiId : (userRow.upi_id || ''),
