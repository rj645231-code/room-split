...
const insMember = db.prepare('INSERT INTO group_members (group_id, user_id) VALUES (?, ?)');
users.forEach(u => insMember.run(groupId, u.id));

// ── Helper: create expense ──────────────────────────────────────────────────────
const createExp = (title, paidByIdx, itemDefs, category = 'grocery', daysAgo = 0) => {
  ...
  items.forEach(item => {
    ...
    const insC = db.prepare('INSERT OR IGNORE INTO item_consumers (item_id, user_id) VALUES (?, ?)');
    item.consumers.forEach(uid => insC.run(itemId, uid));
  });

  // Splits
  const splits = computeSplits(items, paidBy);
  const insSplit = db.prepare('INSERT INTO splits (id, expense_id, user_id, amount) VALUES (?, ?, ?, ?)');
  splits.forEach(s => insSplit.run(uuidv4(), expId, s.user, s.amount));
};

// ── Seed Expenses ───────────────────────────────────────────────────────────────
createExp('Weekly Grocery Run', 0, [
  { name: 'Rice (5kg)',  totalCost: 280, consumers: [0,1,2,3] },
  { name: 'Chicken 1kg',totalCost: 250, consumers: [2]       }, // Only Rahul
  { name: 'Milk 2L',    totalCost: 80,  consumers: [0,2,3]   }, // Priya excluded
  { name: 'Vegetables', totalCost: 180, consumers: [0,1,2,3] },
  { name: 'Bread',       totalCost: 60, consumers: [0,2]     },
], 'grocery', 0);

createExp('Dinner at home', 1, [
...
createExp('Internet Bill', 2, [
  { name: 'Broadband March', totalCost: 799, consumers: [0,1,2,3] },
], 'utility', 3);

createExp('Breakfast supplies', 3, [
